...
greaterThan(QT_MAJOR_VERSION, 4) {
    ...
    # Don't overwrite any existing component gallery example on device
    TARGET = qtquickcomponentgallery
    desktop.files = qtquickcomponentgallery.desktop
} else {
    include(../util/util.pri)
    QT += declarative
    TARGET = qmlcomponentgallery
